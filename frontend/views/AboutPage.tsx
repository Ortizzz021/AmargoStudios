      ...
      <section className="container">
        <AnimateOnScroll>
          ...
          <div className="grid grid-3">
            {team.map((member, idx) => (
              <AnimateOnScroll key={member.name} delay={idx * 150}>
                <div className="team-card">
                  <div className="team-img-container">
                    <Image src={member.image} alt={`${member.name} - ${member.role}`} width={400} height={280} loading="lazy" style={{ objectFit: 'cover' }} />
                  </div>
      ...
      <section className="container">
        <AnimateOnScroll>
          ...
          <div className="vision-mission">
            <div className="vision-card" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: 'var(--spacing-md)' }}>
              <h3 className="mb-md" style={{ color: 'var(--color-accent-light)' }}>Nuestra Visión</h3>
              <p>
            ...
            <div className="mission-card" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: 'var(--spacing-md)' }}>
              <h3 className="mb-md" style={{ color: 'var(--color-accent-light)' }}>Nuestra Misión</h3>
              <p>
              ...
              <AnimateOnScroll key={title} delay={idx * 100}>
                <div className="card">
                  <div className="card-content">
                    <h3 style={{ color: 'var(--color-accent-light)', marginBottom: 'var(--spacing-xs)' }}>{title}</h3>
                    <p>{desc}</p>
                  </div>
                </div>
              </AnimateOnScroll>
